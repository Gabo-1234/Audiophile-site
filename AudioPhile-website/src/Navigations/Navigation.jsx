...
const Navi = () => {
  ...
  const { cartItems, Increase, Decrease, removeFromCart, removeAllFromCart } = useContext(CartContext);
  ...
  const goToCheckout = () => {
    navigate("/checkout");
    const buc = document.getElementById("bucket");
    buc.classList.add("hidden");
  ...
  const showHr = !isProductPath && pathname !== "/checkout";
  const showHeading = pathname !== "/" && !isProductPath && pathname !== "/checkout";
  ...
  return (
    <>
      <div className="Black-bg-Head">
        ...
        {isBucketOpen && <div className="bg-dark" onClick={showBucket}></div>}
        <div className={isBucketOpen ? "bucket" : "hidden bucket"} id="bucket">
        <div className="t">
            <div className="bk-tp">
            <h3 className="h3">cart</h3>
            <button className="removeAll" onClick={removeAllFromCart}>Remove all</button>
          </div>
          <ul className="AddedPr">
            {cartItems.length > 0 ? (
              ...
              <li style={{padding: "20px", textAlign: "center", color: "#999"}}>Cart is empty</li>
            )}
          </ul>
        </div>
          <div className="checkNtotal">
            ...
            <button className="Product-view-btn" style={{ width: "100%" }} onClick={goToCheckout}>
              CHECKOUT
            </button>
          </div>
        </div>
      ...
      <div className={showHeading ? "Tittle-bg" : "Tittle-bg hidden"} id="hid">
        {loca.pathname === "/HeadphonesPage" && <h2>Headphones</h2>}
        {loca.pathname === "/SpeakersPage" && <h2>Speakers</h2>}
        {loca.pathname === "/EarphonesPage" && <h2>Earphones</h2>}
      </div>
    </>
  );
};
